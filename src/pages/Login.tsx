  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (msg.includes('email not confirmed')) {
        toast.error('Email não confirmado', { description: 'Verifique sua caixa de entrada para o link de confirmação.' });
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-[#10B981] hover:text-[#10B981]/80"
                >
                  Esqueci a senha
                </button>
              </div>
        ...
        <DialogContent className="bg-[#0F172A] border-white/10 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/60">
              Enviaremos um link para redefinir sua senha para o email cadastrado.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleForgot} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="forgot-email" type="email" placeholder="voce@empresa.com"
                value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
                required disabled={forgotLoading}
                className="bg-white/5 border-white/10 text-white placeholder:text-white/30 focus-visible:ring-[#10B981]/40 h-11"
              />
            </div>
            <DialogFooter>
              <Button
